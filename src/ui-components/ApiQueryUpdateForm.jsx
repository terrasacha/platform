...
import { getApiQuery } from "../graphql/queries";
import { updateApiQuery } from "../graphql/mutations";
export default function ApiQueryUpdateForm(props) {
  const {
    id: idProp,
    apiQuery: apiQueryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = apiQueryRecord
      ? { ...initialValues, ...apiQueryRecord }
      : initialValues;
    setCedulaCatastral(cleanValues.cedulaCatastral);
    ...
    setFechaHoraConsulta(cleanValues.fechaHoraConsulta);
    setFechaHoraActualizacion(cleanValues.fechaHoraActualizacion);
    setVerificado(cleanValues.verificado);
    setRawConsulta(
      typeof cleanValues.rawConsulta === "string" ||
        cleanValues.rawConsulta === null
        ? cleanValues.rawConsulta
        : JSON.stringify(cleanValues.rawConsulta)
    );
    setResultadoConsulta(
      typeof cleanValues.resultadoConsulta === "string" ||
        cleanValues.resultadoConsulta === null
        ? cleanValues.resultadoConsulta
        : JSON.stringify(cleanValues.resultadoConsulta)
    );
    setErrors({});
  };
  const [apiQueryRecord, setApiQueryRecord] = React.useState(apiQueryModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getApiQuery.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getApiQuery
        : apiQueryModelProp;
      setApiQueryRecord(record);
    };
    queryData();
  }, [idProp, apiQueryModelProp]);
  React.useEffect(resetStateValues, [apiQueryRecord]);
  const validations = {
    cedulaCatastral: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          cedulaCatastral: cedulaCatastral ?? null,
          imgAnteriorSatellite,
          imgAnteriorYear,
          imgAnteriorMesInicial,
          imgAnteriorMesFinal,
          ...
          imgPosteriorMesFinal,
          imgPosteriorNubosidadMaxima,
          fechaHoraConsulta: fechaHoraConsulta ?? null,
          fechaHoraActualizacion: fechaHoraActualizacion ?? null,
          verificado,
          rawConsulta: rawConsulta ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateApiQuery.replaceAll("__typename", ""),
            variables: {
              input: {
                id: apiQueryRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ApiQueryUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || apiQueryModelProp)}

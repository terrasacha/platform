...
import { updateAnalysis } from "../graphql/mutations";
export default function AnalysisUpdateForm(props) {
  const {
    id: idProp,
    analysis: analysisModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = analysisRecord
      ? { ...initialValues, ...analysisRecord }
      : initialValues;
  ...
  const [analysisRecord, setAnalysisRecord] = React.useState(analysisModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getAnalysis.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAnalysis
        : analysisModelProp;
    ...
    queryData();
  }, [idProp, analysisModelProp]);
  React.useEffect(resetStateValues, [analysisRecord]);
  const validations = {
    imgAnteriorNombreImg: [],
    imgAnteriorSatellite: [],
    imgAnteriorYear: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateAnalysis.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "AnalysisUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || analysisModelProp)}

...
import { getFeature } from "../graphql/queries";
import { updateFeature } from "../graphql/mutations";
export default function FeatureUpdateForm(props) {
  const {
    id: idProp,
    feature: featureModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = featureRecord
      ? { ...initialValues, ...featureRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setIsTemplate(cleanValues.isTemplate);
    setIsVerifable(cleanValues.isVerifable);
    setDefaultValue(cleanValues.defaultValue);
    setFormOrder(cleanValues.formOrder);
    setFormHint(cleanValues.formHint);
    setFormRequired(cleanValues.formRequired);
    setFormAppearance(cleanValues.formAppearance);
    setFormRelevant(cleanValues.formRelevant);
    setFormConstraint(cleanValues.formConstraint);
    setFormRequiredMessage(cleanValues.formRequiredMessage);
    setErrors({});
  };
  const [featureRecord, setFeatureRecord] = React.useState(featureModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getFeature.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFeature
        : featureModelProp;
      setFeatureRecord(record);
    };
    queryData();
  }, [idProp, featureModelProp]);
  React.useEffect(resetStateValues, [featureRecord]);
  const validations = {
    name: [{ type: "Required" }],
    description: [],
    isTemplate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          isVerifable: isVerifable ?? null,
          defaultValue: defaultValue ?? null,
          formOrder: formOrder ?? null,
          formHint: formHint ?? null,
          formRequired: formRequired ?? null,
          formAppearance: formAppearance ?? null,
          formRelevant: formRelevant ?? null,
          formConstraint: formConstraint ?? null,
          formRequiredMessage: formRequiredMessage ?? null,
        };
        ...
        try {
          ...
          await API.graphql({
            query: updateFeature.replaceAll("__typename", ""),
            variables: {
              input: {
                id: featureRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "FeatureUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || featureModelProp)}

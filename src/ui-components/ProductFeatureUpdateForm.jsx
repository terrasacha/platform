...
import { getProductFeature } from "../graphql/queries";
import { updateProductFeature } from "../graphql/mutations";
export default function ProductFeatureUpdateForm(props) {
  const {
    id: idProp,
    productFeature: productFeatureModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = productFeatureRecord
      ? { ...initialValues, ...productFeatureRecord }
      : initialValues;
    setValue(cleanValues.value);
    setIsToBlockChain(cleanValues.isToBlockChain);
    setOrder(cleanValues.order);
    setIsOnMainCard(cleanValues.isOnMainCard);
    setIsResult(cleanValues.isResult);
    setErrors({});
  };
  const [productFeatureRecord, setProductFeatureRecord] = React.useState(
    productFeatureModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getProductFeature.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getProductFeature
        : productFeatureModelProp;
      setProductFeatureRecord(record);
    };
    queryData();
  }, [idProp, productFeatureModelProp]);
  React.useEffect(resetStateValues, [productFeatureRecord]);
  const validations = {
    value: [],
    isToBlockChain: [],
    order: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          value: value ?? null,
          isToBlockChain: isToBlockChain ?? null,
          order: order ?? null,
          isOnMainCard: isOnMainCard ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateProductFeature.replaceAll("__typename", ""),
            variables: {
              input: {
                id: productFeatureRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ProductFeatureUpdateForm")}
      {...rest}
    >
      <TextField
        label="Value"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || productFeatureModelProp)}

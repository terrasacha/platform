...
export default function TokenDistributionInputTable(props) {
  ...
  const handleSaveHistoricalData = async (indexToSave) => {
    ...
    let revenueByProductToUpload = revenuesByProduct.map(rbp =>{return{CONCEPTO: rbp.CONCEPTO, CANTIDAD: rbp.CANTIDAD}})
    if (
    ...
    <>
      <Card className={className}>
        ...
        <Card.Body>
          ...
          <div>
            <TableEdit canEdit={canEdit} conceptOptions={conceptOptions} columns={['CONCEPTO', 'CANTIDAD']} infoTable={revenuesByProduct} handleEditValue={handleEditValue} handleChangeInputValue={handleChangeInputValue} handleAddCashFlow={handleAddCashFlow} handleSaveHistoricalData={handleSaveHistoricalData} handleDeleteHistoricalData={handleDeleteHistoricalData}/>
          </div>
        </Card.Body>
      </Card>
    </>

...
export default function NewCampaign() {
  ...
  const handleSubmit = async (e) => {
    ...
    const timestampData = {
      ...formData,
      initialDate: formData.initialDate ? Math.floor(new Date(formData.initialDate).getTime() / 1000) : '',
      endDate: formData.endDate ? Math.floor(new Date(formData.endDate).getTime() / 1000) : ''
    };
    
    try {
      const result = await API.graphql(graphqlOperation(createCampaign, { input: timestampData }));
      ...
      if (images.length > 0) {
        for (let image of images) {
          ...
          const imageUrlObj = `${process.env.REACT_APP_URL_BUCKET}/public/campaign/${campaignId}-campaign/${encodeURIComponent(image.name)}`

  ...
  import { FaFileUpload, FaTrash, FaEye, FaEdit, FaCheckCircle, FaExclamationTriangle } from "react-icons/fa";
  ...
  export default function ValidationModal({ isOpen, onClose , onValidationComplete}) {
    ...
    const isUploadDisabled = !["PENDING", "NOT_SELECTABLE", "DOC_UPLOADED"].includes(propertyData?.propertyInfo?.status) 
    || !!propertyData?.propertyInfo?.userLegalID;
  ...
  const uploadFiles = async () => {
    ...
    try {
        ...
        await Promise.all(
            Object.entries(selectedFiles).map(async ([fileType, file]) => {
                ...
                if (uploadedFiles[fileType]) {
                    console.log(`🗑️ Eliminando archivo existente antes de subir el nuevo: ${uploadedFiles[fileType]}`);
        ...
        setUploadProgress((prev) => {
            const failedFiles = Object.keys(selectedFiles).reduce((acc, fileType) => {
                acc[fileType] = "error";
                return acc;
            }, {});
  ...
  const deleteS3File = async (fileKey, fileType) => {
    ...
    <div key={fileType} className="flex items-center justify-between border p-3 rounded-md shadow-sm mb-3">
        ...
        <div className="flex items-center gap-2">
            ...
            {/* 🔹 Botón "Editar" si el archivo está seleccionado (incluso antes de guardar cambios) */}
